...
def save_to_db() -> None:
    ...
    try:
        while running:
            ...
            if redis_ids:
                logger.debug(f'Redis group processing {len(redis_ids)} requests')
            ...
            with conn.cursor() as cur:
                try:
                    ...
                    logger.debug(f'Redis group completed processing {len(redis_ids)} requests')
                except:
                    conn.rollback()
                    logger.error(f'Redis group failed to process {len(redis_ids)} requests, from request ID {redis_ids[0]} to {redis_ids[-1]}')
                    raise
                finally:
                    pool.putconn(conn)
    finally:
